...
if __name__ == '__main__':
    ...
    if question == '1.2':
        ...
        # TODO: your code here
        # Part 1
        print("Part 1: " + wordlist[50])
        print("Part 2: " + str(wordlist[X[500, :] == 1]))
        print("Part 3: " + str(groupnames[y[500]]))
        ...
        model = NaiveBayes(num_classes=4, beta=1.0)
        model.fit(X, y)
        y_pred = model.predict(X_valid)
        v_error = np.mean(y_pred != y_valid)
        ...
        print("Time taken for our random forest: %.3f" % (time.time() - t))
        evaluate_model(RandomForest(max_depth=10, num_trees=50))
        ...
        print("Time taken for scikit-learn random forest: %.3f" % (time.time() - t))
        ...
        fname = os.path.join("..", "figs", "error_vs_k_3_2.png")
        plt.savefig(fname)
        print("\nFigure saved as '%s'" % fname)
        ...
        # plt.show()
        fname = os.path.join("..", "figs", "clusterdata_dbscan_4.png")
        plt.savefig(fname)
        print("\nFigure saved as '%s'" % fname)
        ...
        for b in [1, 2, 4, 6]:
            ...
            plt.figure()
            plt.imshow(quantizer.colours[None] if b / 2 != b //
                       2 else np.reshape(quantizer.colours, (2**(b // 2), 2**(b // 2), 3)))
