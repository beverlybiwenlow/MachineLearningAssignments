...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        # TODO
        model = LogisticRegression(penalty='l2', C=1, fit_intercept=False)
        model.fit(XBin, yBin)

        print("\nScikit L2 Training error %.3f" %
              utils.classification_error(model.predict(XBin), yBin))
        print("Scikit L2 Validation error %.3f" %
              utils.classification_error(model.predict(XBinValid), yBinValid))
        print("Scikit L2 # nonZeros: %d" % (model.coef_ != 0).sum())

        model = LogisticRegression(penalty='l1', C=1, fit_intercept=False)
        model.fit(XBin, yBin)

        print("\nScikit L1 Training error %.3f" %
              utils.classification_error(model.predict(XBin), yBin))
        print("Scikit L1 Validation error %.3f" %
              utils.classification_error(model.predict(XBinValid), yBinValid))
        print("Scikit L1 # nonZeros: %d" % (model.coef_ != 0).sum())
        ...
        model = LogisticRegression(multi_class='multinomial',
                                   solver='lbfgs', fit_intercept=False, C=1000)
        model.fit(XMulti, yMulti)
